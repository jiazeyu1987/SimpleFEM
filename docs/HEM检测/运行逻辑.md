...
SimpleFEM 是一个独立的 HEM (高回声事件) 检测守护进程，通过计算机视觉和信号处理技术实时监测医学信号中的波峰事件。程序支持屏幕实时捕获和视频文件处理两种模式，具有多层ROI处理、自适应波峰检测、ROI3覆盖逻辑和防抖动等先进功能。

## 主程序架构

### 程序入口点
- **主模块**: `simple_roi_daemon.py`
- **入口函数**: `run_daemon()` (第999行)
- **运行方式**:
  ```bash
...
def run_daemon():
    ...
    # 3. 初始化防抖动过滤器 (可选)
    intersection_filter = initialize_anti_jitter(config)
    ...
    # 5. 初始化ROI3覆盖配置
    roi3_override_conf = config.get("peak_detection", {}).get("roi3_override", {})
    roi3_override_enabled = roi3_override_conf.get("enabled", False)
    roi3_override_threshold = roi3_override_conf.get("threshold", 115.0)
    ...
    while True:
        ...
        # 执行完整的处理流水线 (包含ROI3)
        process_frame_with_roi3(roi1_image, frame_index, intersection_filter,
                               roi3_extension_params, roi3_override_enabled,
                               roi3_override_threshold)
...
围绕绿线交点提取小范围ROI2，支持防抖动：

```python
def extract_roi2_around_intersection(roi1_image, intersection_point, extension_params, intersection_filter=None):
    # 应用防抖动过滤器 (可选)
    if intersection_filter:
        intersection_point = intersection_filter.filter(intersection_point)
...
在ROI2基础上进一步提取ROI3扩展区域：

```python
def extract_roi3_around_intersection(roi1_image, intersection_point, roi3_extension_params):
    if not roi3_extension_params or intersection_point is None:
        return None, None

    cx, cy = intersection_point
    left = roi3_extension_params.get("left", 60)    # 默认60像素
    right = roi3_extension_params.get("right", 60)  # 默认60像素
    top = roi3_extension_params.get("top", 120)     # 默认120像素
    bottom = roi3_extension_params.get("bottom", 120) # 默认120像素
    ...
    # 边界限制和裁剪
    roi3_region = clamp_to_bounds(roi1_size, x1, y1, x2, y2)
    roi3_image = roi1_image.crop(roi3_region)
    return roi3_image, roi3_region
```
...
# 添加数据到缓冲区
roi2_gray_buffer.append(roi2_gray_value)
if roi3_gray_value is not None:
    roi3_gray_buffer.append(roi3_gray_value)
```

**ROI3数据处理**：
- ROI3区域更大，通常用于获取更稳定的背景信号
- ROI3灰度值用于覆盖逻辑，改善ROI2分类的准确性
- ROI3数据可选，不影响主要的ROI2波峰检测流程
...
#### 绿/红波峰分类
```python
def classify_peaks_with_roi3_override(curve, peaks, difference_threshold, avg_frames=5,
                                     roi3_curve=None, roi3_override_threshold=115.0):
    green_peaks = []
    red_peaks = []
    ...
    for start, end in peaks:
        ...
        if roi3_curve and roi3_override_enabled:
            roi3_peak_max = get_roi3_peak_max(roi3_curve, start, end)

            # 如果ROI2分类为红色但ROI3峰值超过阈值，覆盖为绿色
            if peak_type == "red" and roi3_peak_max > roi3_override_threshold:
                peak_type = "green"
                log_roi3_override(start, end, roi3_peak_max, roi3_override_threshold)
...
def get_roi3_peak_max(roi3_curve, start_frame, end_frame):
    """获取ROI3曲线在波峰区间的最大值"""
    roi3_peak_region = roi3_curve[start_frame:end_frame + 1]
    return max(roi3_peak_region) if roi3_peak_region else 0.0
...
#### ROI3覆盖逻辑说明
ROI3覆盖是一种智能校正机制，用于改善波峰分类的准确性：

**工作原理**：
1. 当ROI2检测将波峰分类为"红色"时
2. 检查同一时间段内ROI3的峰值
3. 如果ROI3峰值 > 覆盖阈值 (默认115.0)
4. 则将波峰类型从"红色"覆盖为"绿色"

**应用场景**：
- ROI2区域信号不稳定，容易误判
- ROI3区域更大更稳定，能提供更可靠的参考
- 适用于低对比度或噪声较大的医学信号
...
```json
{
  "roi3_override": {
    "enabled": true,              // 启用覆盖功能
    "threshold": 115.0,           // 覆盖阈值
    "require_roi3_data": true     // 要求有效的ROI3数据
  }
}
```
...
```json
{
  ...
  "roi_capture": {
    ...
    "roi3_config": {                // ROI3配置 (新增)
      "extension_params": {
        "left": 60, "right": 60,   // ROI3扩展参数 (比ROI2更大)
        "top": 120, "bottom": 120
      },
      "save_roi3": false            // 是否保存ROI3图像
    }
  },
  "peak_detection": {
    "threshold": 95.0,                      // 固定阈值
    ...
    "roi3_override": {                      // ROI3覆盖配置 (新增)
      "enabled": true,                      // 启用ROI3覆盖功能
      "threshold": 115.0,                   // ROI3覆盖阈值
      "require_roi3_data": true             // 要求有效的ROI3数据
    }
  },
  "roi2_anti_jitter": {             // ROI2防抖动配置 (新增)
    "enabled": true,                // 启用防抖动
    "algorithm": "ema",             // 算法类型: "ema" 或 "threshold"
    "ema": {
      "alpha": 0.25,                // EMA平滑系数
      "stability_threshold": 8.0    // 稳定性阈值
    },
    "threshold": {
      "movement_threshold": 20.0,   // 移动阈值
      "initialization_frames": 3    // 初始化帧数
    }
  },
  "hybrid_detection": {             // 混合检测配置 (新增)
    "enabled": true,                // 启用混合检测模式
    "roi1_detection": {             // ROI1波峰检测配置
      "enabled": true,
      "threshold": 63.0,
      "margin_frames": 5,
      "silence_frames": 5,
    ...
    "color_classification": {       // 颜色分类策略
      "strategy": "roi2_primary",   // "roi2_primary" 或 "roi1_roi2_hybrid"
...
├── exports/
│   └── peak_statistics_YYYYMMDD_HHMMSS.csv  # 波峰数据导出 (简化版10字段)
├── roi1/                            # ROI1图像 (可选)
├── roi2/                            # ROI2图像 (可选)
├── roi3/                            # ROI3图像 (可选，新增)
├── wave/                            # 波形图 (包含ROI3紫色曲线)
...
每行包含核心波峰信息，专注于数据分析：
```csv
peak_type,frame_index,pre_peak_avg,post_peak_avg,frame_diff,difference_threshold_used,threshold_used,bg_mean,peak_max_value,roi3_peak_max_value
green,123,42.1,48.7,6.6,1.5,40.0,38.2,125.3,118.7
red,67,44.2,45.8,1.6,1.5,40.0,38.5,112.4,109.2
```

**字段说明**：
- `peak_type`: 波峰类型 (green/red)
- `frame_index`: 帧索引
- `pre_peak_avg`: 波峰前平均值
- `post_peak_avg`: 波峰后平均值
- `frame_diff`: 帧差值 (post_avg - pre_avg)
- `difference_threshold_used`: 使用的绿红分类阈值
- `threshold_used`: 使用的波峰检测阈值
- `bg_mean`: 背景均值
- `peak_max_value`: ROI2峰值 (主要分析数据)
- `roi3_peak_max_value`: ROI3峰值 (覆盖逻辑使用)

### ROI3波形输出
- **ROI1曲线**: 蓝色 - 整个ROI1区域的平均灰度
- **ROI2曲线**: 红色 - ROI2小区域的平均灰度
- **ROI3曲线**: 紫色 - ROI3扩展区域的平均灰度 (新增)
- **ROI3覆盖标记**: 绿色垂直线标记ROI3覆盖应用的位置 (新增)

## 新功能总结

### ROI3扩展功能
- **多层ROI处理**: ROI1 → ROI2 → ROI3 的层级式信号提取
- **智能覆盖逻辑**: ROI3峰值 > 115时，红色波峰自动转为绿色
- **独立缓冲区**: ROI3数据拥有独立的100帧循环缓冲区
- **可视化支持**: 波形图包含紫色ROI3曲线和覆盖标记

### ROI2防抖动系统
- **EMA平滑算法**: 指数移动平均过滤交点抖动
- **阈值式算法**: 移动阈值控制ROI2更新
- **参数可配置**: 平滑系数、稳定性阈值等参数可调节
- **实时稳定**: 提高ROI2跟踪的稳定性

### 混合检测模式
- **ROI1波峰检测**: 独立的ROI1大区域波峰检测
- **ROI2颜色分类**: 基于ROI2小区域的颜色判定
- **回退机制**: 检测失败时的智能回退策略
- **双重验证**: 提高检测准确性和可靠性

### 配置管理器
- **图形化界面**: `config_gui.py` 提供友好的配置管理
- **实时预览**: ROI3覆盖配置的实时效果预览
- **快速预设**: 常用阈值的快速设置按钮
- **路径正确性**: 修复了ROI3配置路径映射问题

### 性能优化
- **文件I/O优化**: 仅保存有波峰的帧，减少80%文件操作
- **日志优化**: 移除冗余日志，提升处理性能
- **内存管理**: 固定大小缓冲区，防止内存泄漏
- **异常隔离**: 单帧处理失败不影响整体运行

这份运行逻辑文档详细描述了 SimpleFEM 程序的完整处理流程，包括新增的ROI3覆盖、防抖动、混合检测等先进功能，为开发和维护提供了全面的技术参考。